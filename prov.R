...
QUA CHE ERA IL MIGLIORE HO PROVATO A LASVCIARE ANCHE IL DATASET ELIMINANDO
I MISSIN E SENZA IMPUTAZIONE, MA EFFETTIVAMENTE NON SONO DIRETTAMENTE
CONFROTNABILI. PERò POTREMMO COMUNQUE MEYYERLO
...
my_training2 <- mice(train, method = "mean", predictorMatrix = my_predictorMatrix, seed = 1234,  printFlag = FALSE)
my_training3 <- mice(train, method = "norm", predictorMatrix = my_predictorMatrix, seed = 1234,  printFlag = FALSE)
my_training4 <- na.omit(train)


model1 <- glm.mids(Dropout ~ ., family="binomial", data = my_training1)
model2 <- glm.mids(Dropout ~ ., family="binomial", data = my_training2)
model3 <- glm.mids(Dropout ~ ., family="binomial", data = my_training3)
model4 <- glm(Dropout ~ ., family="binomial", data = my_training4)
...
mod4 <- predict(model4, test, type="response")
final_pred_class4 <- ifelse(mod4 > 0.5, 1, 0)
confusion_matrix4 <- table(test$Dropout, final_pred_class4)
accuracies[4] <- sum(diag(confusion_matrix4)) / sum(confusion_matrix4)
f1[4] <- f1_score(confusion_matrix4)
# Risultati
accuracies
...
cv_threshold_model <- function(data, formula, model_type = "glm", k = 5, soglia_seq = seq(0.1, 0.9, 0.01)) {
  ...
  f1_cv <- matrix(NA, nrow = k, ncol = length(soglia_seq))
  
  for (fold in 1:k) {
    
    train_fold <- data[folds != fold, ]
    val_fold   <- data[folds == fold, ]
    
    # --- FIT MODEL ---
    if (model_type == "glm") {
      model <- glm(formula, data = train_fold, family = "binomial")
      probs <- predict(model, val_fold, type = "response")
      
    } else if (model_type == "lda") {
      model <- MASS::lda(formula, data = train_fold)
      probs <- predict(model, val_fold)$posterior[,2]
      
    } else if (model_type == "qda") {
      model <- MASS::qda(formula, data = train_fold)
      probs <- predict(model, val_fold)$posterior[,2]
      
    } else if (model_type == "rf") {
      model <- randomForest::randomForest(formula, data = train_fold, ntree = 200)
      probs <- predict(model, val_fold, type = "prob")[,2]
      
    } else {
      stop("Modello non supportato")
    }
    
    # --- THRESHOLD LOOP ---
    for (i in seq_along(soglia_seq)) {
      pred <- ifelse(probs > soglia_seq[i], 1, 0)
      cm <- table(val_fold$Dropout, pred)
      f1_cv[fold, i] <- f1_score(cm)
    }
  }
  
  mean_f1 <- colMeans(f1_cv, na.rm = TRUE)
  best_idx <- which.max(mean_f1)
  
  list(
    best_threshold = soglia_seq[best_idx],
...
# DEVO OTTIMIZZARE LA SOGLIA ANCHE IN LDA QDA E RANDOM FOREST
